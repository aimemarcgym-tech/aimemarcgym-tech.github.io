...
function VideoTagsEditor({ video, onChange }: { video: Video; onChange: () => void }) {
  ...
  async function save() {
    const tags = value
      ...
      .map((t) => t.trim())
      .filter(Boolean);
    await setVideoTags(video.id, tags);
    setSaved(true);
    onChange();
  }
...
function VideoCard({ video, onChange }: { video: Video; onChange: () => void }) {
  ...
  const objectUrl = useMemo(() => URL.createObjectURL(video.blob), [video]);
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border-subtle bg-surface-alt/40">
      ...
      <video controls src={objectUrl} className="aspect-video w-full bg-black" />
      <div className="p-2">
...
function AlbumView({
  ...
  function refresh() {
    getVideosByAlbum(album.id).then(setVideos);
  }
  ...
  async function handleDeleteAlbum() {
    if (!confirm(`Supprimer l'album « ${album.name} » et toutes ses vidéos ?`)) return;
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {uploading ? "Import…" : "+ Ajouter des vidéos"}
          </button>
          <button
            type="button"
      ...
      {!videos ? (
        ...
        <p className="text-sm text-muted">Aucune vidéo dans cet album pour l&apos;instant.</p>
      ) : (
...
export default function VideoAlbumManager() {
