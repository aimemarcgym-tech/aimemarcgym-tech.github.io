...
export default function BackupPanel() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      downloadBackup(data);
      setStatus(
        `Sauvegarde téléchargée (${data.gymnasts.length} gymnaste(s), ${data.movements.length} mouvement(s)).`
      );
    } catch (e) {
  ...
  async function confirmImport() {
    ...
    try {
      ...
      setStatus(
        `Sauvegarde importée : ${counts.gymnasts} gymnaste(s), ${counts.movements} mouvement(s), ${counts.clubs} club(s). Les données précédentes de cet appareil ont été remplacées.`
      );
    } catch (e) {
    ...
    <div className="space-y-6">
      <div className="rounded-lg border border-border-subtle bg-surface p-4">
        ...
        <p className="mb-3 text-xs text-muted">
          ...
          mouvements, historique). À faire régulièrement, en particulier avant/après une compétition.
        </p>
        <button
